...
```{r}
set.seed(123)
hc <- 13 + rbinom(45, 5, 0.2)
ri <- 13 + rbinom(36, 5, 0.2)
an <- 13 + rbinom(40, 7, 0.2)

edu_df <- data.frame(
  edu = c(hc, ri, an),
  group = c(rep("hc", 45), rep("ri", 36), rep("an", 40))
)
...
delta_t <-
    ...
    posterior_samples(m0, pars = c("^b_", "sd_", "sigma")) %>% 
...
@shahar2019credit investigated how spatial-motor associations impact reinforcement learning (RL). In RL, optimal decision-making prioritizes rewards, regardless of previous choices. However, @shahar2019credit found that rewards had a greater impact on the probability of choosing between two images when the chosen image was linked to the same response key in both the previous and current trials.

In order to investigate whether this effect was also present in our data, we reproduced the statistical evaluations conducted by by @shahar2019credit and @ben2022working. We found that the likelihood of selecting "stay" was greater for "same" response/key mappings than for "flipped" mappings, both for rewarded and unrewarded responses. 
